...
| Показываются пары чужой подгруппы | Метка подгруппы не дошла до фильтра | `probe_site.py` покажет, сколько пар с пометкой пришло со страницы и из JSON-API. Пометка есть только на странице; если и там ноль — сменилась вёрстка, правится `bot/timetable/scraper.py` |
| Названия занятий по-русски при английском интерфейсе | Расписание пришло из JSON-API: он не переводит | В логе будет «Страница расписания недоступна… пробуем JSON-API». Проверьте, открывается ли страница расписания с сервера |
